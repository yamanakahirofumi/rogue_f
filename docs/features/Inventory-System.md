...
- **処理レスポンス (`UseItemResult`)**: アイテム消費の有無 (`consumed`)、HP/ST/満腹度の回復数値、付与/解除された状態異常リスト、鑑定結果、テレポート先座標等が構造化されてクライアントに返却されます（[実装詳細](../implementation/Implementation-Details.md) 参照）。

### 3.4 アイテムのドロップおよび投擲
- 不要なアイテムを足元に「置く（`drop`）」または「投げる（`throw`）」ことができます。
- **ドロップレスポンス (`DropItemResult`)**: 置かれた足元座標が返却されます。
- **投擲レスポンス (`ThrowResult`)**: 対象への命中・ダメージ判定、投げた薬等の破砕・消滅 (`isBroken`)、最終的な着地座標、付与された状態異常効果が返却されます（[実装詳細](../implementation/Implementation-Details.md) 参照）。
