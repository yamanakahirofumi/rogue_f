...
## 3. 地形の撤去・解体（標準床へのリセット）による資材・ゴールドの回収

管理者が配置済みの特殊地形・構造物をダンジョン編集画面から撤去・解体して標準の「床 (`floor`)」へ戻した際、回収される資材およびゴールド（設置コストの 50%、端数切り捨て）の一覧です。

| typeId | 名称 | 回収される資材 | 回収されるゴールド | 備考 |
| :--- | :--- | :--- | :--- | :--- |
| `floor` | 床 | なし | 0 | 基本地形のため解体・回収不可 |
| `wall` | 壁 | 石材 (`stone`) × 1 | 0 | 設置コスト: 石材 × 2 |
| `door` | 扉 | 木材 (`wood`) × 2, 鉄材 (`iron`) × 0 | 0 | 設置コスト: 木材 × 5, 鉄材 × 1 |
| `water` | 水 | なし | 0 | 設置コスト: 魔力結晶 (`magic_crystal`) × 1 (50%端数切捨てで0) |
