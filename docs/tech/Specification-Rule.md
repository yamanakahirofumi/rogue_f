...
- 仕様策定時に必要なデータ構造が明確になった場合は、`src/@types/` 配下に型定義を作成し、仕様書からもその型を参照するようにしてください。
...
| `Optimization-Strategy.md` | パフォーマンスを維持するための最適化手法（カリング、計算量削減、WebWorker 活用等）。 |
...
### プルリクエスト (PR) のタイトル
ドキュメントの更新を含む PR を作成する際は、タイトルの冒頭に以下のいずれかのプレフィックスを付与してください。
- `[追加]`: 新しい仕様や実装詳細の追加。
- `[提案]`: 新機能や改善案の提案。
- `[整理]`: 既存情報の整理、バージョンの更新、誤記修正。
