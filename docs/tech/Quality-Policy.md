...
## 4. 運用
- CI は常に有効化し、GitHub Actions によりビルド/テスト/カバレッジを自動実行（[CI 設定](CI-Setting.md) 参照）。
- カバレッジレポートは `coverage/` ディレクトリ配下の出力を標準とする。
- テスト設計・記述は [テストルール](Test-Rule.md) に準拠。
