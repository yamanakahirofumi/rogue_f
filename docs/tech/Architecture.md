...
## 4. Angular の適用範囲とドメイン分離の原則

本プロジェクトにおける Angular の使用は、SPA としての役割（画面描画・イベント管理・DI/サービス連携）に限定します。ドメイン（ゲームルール、エンティティ、状態遷移などのビジネスロジック）は Angular から独立した「純粋な TypeScript」として実装します。

- 適用範囲（Angular）
  - UI の描画（Component/Template/Style）
  - ユーザーイベントのハンドリング（Component 側での入力受付）
  - DI コンテナ（`@Injectable` な Service）による依存性管理と API 呼び出し、アプリケーション層の調停
  - ルーティング、モジュール構成、RxJS によるストリーム管理
- 適用範囲（Pure TypeScript = ドメイン）
  - ルールや計算、状態遷移を担うモデル/値オブジェクト/ドメインサービス
  - ファイル配置の推奨: `src/app/fields/d/` など Angular に依存しないディレクトリ
  - 禁則: ドメイン層のコードで Angular 由来の import（`@angular/*`）やブラウザ API に直接依存しない
- 依存方向
  - Component/Service → ドメイン（呼び出しは可）
  - ドメイン → Component/Service（禁止）

この原則により、UI フレームワーク変更の影響を最小化し、テスト容易性と再利用性を高めます。
